...
import 'package:shared/shared.dart' show AppDatabase, WorkingProgressData, Expenditure, RentalItem, TradingEntry, Reminder;
...
class ReportRepositoryImpl implements ReportRepository {
    ...
    try {
      ...
      for (final row in result) {
        ...
        entries.add(
          TradingEntry(
            id: data['id']?.toString() ?? '',
            entryType: entryTypeValue ?? '', // Simple string with null safety
            date: DateTime.tryParse(data['date']?.toString() ?? '') ?? DateTime.now(),
            personName: data['person_name']?.toString() ?? '',
            mobileNo: data['mobile']?.toString() ?? '',
            estateName: '', // Not in report query
            quantity: data['quantity'] != null ? double.tryParse(data['quantity'].toString()) ?? 0.0 : 0.0,
            unitPrice: data['unit_price'] != null ? double.tryParse(data['unit_price'].toString()) ?? 0.0 : 0.0,
            companyId: data['company_id']?.toString() ?? '',
            isActive: true,
            isSynced: true,
            createdAt: DateTime.tryParse(data['date']?.toString() ?? '') ?? DateTime.now(),
            updatedAt: DateTime.tryParse(data['date']?.toString() ?? '') ?? DateTime.now(),
  ...
  Future<Map<String, dynamic>> getReportSummary({
    ...
    try {
      ...
      summary['completedTradingEntries'] = tradingData.length; // Remove status filter since TradingEntry doesn't have status field
      summary['totalTradingValue'] = tradingData.fold<double>(
        0.0,
        (sum, e) => sum + (e.quantity * 1000), // Simplified calculation
      );
