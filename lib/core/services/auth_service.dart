...
class AuthService {
  ...
  static Future<void> _upgradeMissingHashes(Map<String, dynamic> users) async {
    bool mutated = false;
  ...
  static Future<Map<String, dynamic>?> _readUserFromDbByEmailOrUsername(String emailKey) async {
 ...
 static dynamic _parsePermissionsFromDb(dynamic permissions) {
  ...
  static Future<bool> _ensureFirebaseAuthReady() async {
  ...
  static Future<int> syncUsersFromFirestore() async {
    if (kIsWeb) return 0;
  ...
  static Future<int> _syncUsersFromFirestoreInternal() async {
  ...
  static dynamic _cleanPermissions(dynamic perms) {
  ...
  static Future<bool> deleteCorruptedUsersFile() async {
  ...
  static Future<void> _writeFileWithRetry(io.File file, String content) async {
    const maxRetries = 3;
  ...
  // Session Management
  static Future<Map<String, dynamic>> _readSessions() async {
    if (kIsWeb) return {};
    try {
  ...
  static Future<void> _writeSessions(Map<String, dynamic> sessions) async {
    if (kIsWeb) return;
    final file = io.File('${(await _getAppDir()).path}${io.Platform.pathSeparator}$_sessionsFile');
  ...
  // Reset Codes
  static Future<Map<String, dynamic>> _readResetCodes() async {
    if (kIsWeb) return {};
    try {
  ...
  static Future<void> _writeResetCodes(Map<String, dynamic> codes) async {
    if (kIsWeb) return;
    final file = io.File('${(await _getAppDir()).path}${io.Platform.pathSeparator}$_resetCodesFile');
  ...
  static String _generateJWT(String userId, String email, {bool rememberMe = false}) {
  ...
  static bool _verifyJWT(String token) {
    try {
  ...
  static Future<Map<String, dynamic>> login({
        ...
        if (dbResult.isNotEmpty) {
          ...
          if (passwordHash != null) {
            ...
            if (passwordValid) {
              ...
              debugPrint('Password verification failed for database user - attempting Firebase Auth fallback');
              debugPrint('  Entered password length: ${password.length}');
              debugPrint('  Stored hash: ${passwordHash.length > 30 ? passwordHash.substring(0, 30) + "..." : passwordHash}');
              // CRITICAL FIX: Don't return failure here - let Firebase Auth try instead
              // Set user to null to force Firebase Auth attempt
              user = null;
            }
          } else {
            ...
            try {
              ...
              user = {
                ...
                'isActive': dbUser['is_active'] ?? dbUser['isActive'],
                'is_active': dbUser['is_active'] ?? dbUser['isActive'],
                'permissions': _parsePermissionsFromDb(dbPermissions) ?? {'role': 'agent'},
              };
              users[emailKey] = user!;
              await _writeUsers(users);
    ...
    if (user == null) {
      debugPrint('User not found in users map or database - attempting Firebase Auth fallback');
      if (isBypassUser) {
      ...
      // CRITICAL FIX: Don't return failure here - let Firebase Auth attempt authentication
      // Continue to Firebase Auth section below
    }
...
_emitUserUpdate(user);
    ...
    final isAgentOrCompanyAdmin = local.RoleUtils.isAgent(user) || local.RoleUtils.isCompanyAdmin(user);
      ...
      if (!kIsWeb) {
        try {
          ...
          if (dbUser != null && dbHash != null && PasswordHasher.verify(password.trim(), dbHash.trim())) {
            ...
            debugPrint('Password verification failed! - attempting Firebase Auth fallback');
          ...
          debugPrint('Password verification failed and DB fallback errored: $e - attempting Firebase Auth fallback');
          // CRITICAL FIX: Don't return failure here - let Firebase Auth try instead
          // Set user to null to force Firebase Auth attempt
          user = null;
        }
  ...
  static Future<void> _syncOfflineUsersToFirebaseAuth(Map<String, dynamic> usersCache) async {
    if (kIsWeb) return;
    if (_isWindows) return;
    if (Firebase.apps.isEmpty) return;
    try {
  ...
  static Future<void> triggerBackgroundSyncAfterLogin() async {
  ...
  static Future<void> _syncCompaniesFromFirestoreBackground() async {
    if (_isWindows) {
  ...
  static Future<void> _syncOtherCollectionsBackground() async {
    if (_isWindows) {
  ...
  static Future<void> _pushLocalDataToFirestore(Map<String, dynamic> user) async {
    if (_isWindows) return;
  ...
  static bool _verify2FACode(String code, String secret) {
  ...
  // Setup 2FA
  static Future<Map<String, dynamic>> setup2FA(String email, String secret) async {
    final users = await _readUsers();
  ...
  // Get Active Sessions
  static Future<List<Map<String, dynamic>>> getActiveSessions(String userId) async {
    final sessions = await _readSessions();
    final now = DateTime.now();
  ...
  static Future<void> _revokeSessionsByUserId(String userId) async {
  ...
  // Revoke All Sessions
  static Future<void> revokeAllSessions(String userId) async {
    final sessions = await _readSessions();
    sessions.removeWhere((key, value) => value['userId'] == userId);
    await _writeSessions(sessions);
  }
  ...
  static Future<void> _clearFirestorePersistence() async {
    if (kIsWeb) return;
    if (_isWindows) return;
    if (Firebase.apps.isEmpty) return;
    try {
